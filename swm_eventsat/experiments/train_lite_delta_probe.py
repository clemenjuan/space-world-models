"""Train a delta/event decoder for EventSat-Lite latent predictions."""
from __future__ import annotations

import argparse
import math
import sys
import time
from dataclasses import dataclass
from datetime import datetime, timezone
from pathlib import Path
from typing import Any

import numpy as np
import torch
from torch import nn
from torch.utils.data import DataLoader, TensorDataset


ROOT = Path(__file__).resolve().parents[2]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))
SCRIPTS = ROOT / "scripts"
if str(SCRIPTS) not in sys.path:
    sys.path.insert(0, str(SCRIPTS))

from swm_eventsat.data.toy_eventsat_env import STATE_NAMES
from swm_eventsat.data.eventsat_lite_env import LITE_MODE_LIST
from swm_eventsat.models.checkpoint_io import (
    FIGURES,
    STATE_INDEX,
    EventSatStateDecoder,
    fit_normalizers,
    load_eventsat_model,
    normalize_action,
    normalize_obs,
    relpath,
    write_json,
)


DATASET = ROOT / "data/cache/eventsat_lite_trajectories.npz"
OUT = FIGURES / "eventsat_lite_delta_decoder.pt"
METRICS_OUT = FIGURES / "eventsat_lite_delta_decoder_metrics.json"
MODEL_DATASET = "data/cache/eventsat_lite_trajectories.npz"
DELTA_STATE_NAMES = (
    "battery_soc",
    "obc_data_mb",
    "jetson_raw_mb",
    "jetson_compressed_mb",
    "data_downlinked_mb",
)
CONTINUOUS_TARGETS = tuple(f"delta_{name}" for name in DELTA_STATE_NAMES) + ("reward",)
BINARY_TARGETS = (
    "event_observation",
    "event_detection",
    "event_downlink",
    "storage_pressure",
    "low_soc",
    "ground_pass_active",
)
TARGET_NAMES = CONTINUOUS_TARGETS + BINARY_TARGETS


@dataclass
class FeatureBundle:
    pred_x: np.ndarray
    target_x: np.ndarray
    target_y: np.ndarray
    zero_y: np.ndarray
    episode: np.ndarray
    step: np.ndarray
    mode: np.ndarray
    scenario: np.ndarray
    history_size: int
    num_preds: int
    pred_len: int


def _timer() -> float:
    return time.perf_counter()


def _elapsed(start: float) -> float:
    return time.perf_counter() - start


def _fit_standardizer(values: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    mean = values.mean(axis=0).astype(np.float32)
    std = values.std(axis=0).astype(np.float32)
    std[std < 1e-8] = 1.0
    return mean, std


def _build_target(
    state_final: np.ndarray,
    state_prev: np.ndarray,
    reward: np.ndarray,
    storage_capacity_mb: float = 4096.0,
) -> np.ndarray:
    deltas = []
    for name in DELTA_STATE_NAMES:
        idx = STATE_INDEX[name]
        deltas.append(state_final[:, idx] - state_prev[:, idx])
    delta_arr = np.stack(deltas, axis=1).astype(np.float32)
    obs_event = (state_final[:, STATE_INDEX["total_observation_s"]] - state_prev[:, STATE_INDEX["total_observation_s"]] > 0.0)
    detect_event = (state_final[:, STATE_INDEX["total_detections"]] - state_prev[:, STATE_INDEX["total_detections"]] > 0.0)
    downlink_event = (state_final[:, STATE_INDEX["data_downlinked_mb"]] - state_prev[:, STATE_INDEX["data_downlinked_mb"]] > 1e-6)
    stored = (
        state_final[:, STATE_INDEX["obc_data_mb"]]
        + state_final[:, STATE_INDEX["jetson_raw_mb"]]
        + state_final[:, STATE_INDEX["jetson_compressed_mb"]]
    )
    binary = np.stack(
        [
            obs_event.astype(np.float32),
            detect_event.astype(np.float32),
            downlink_event.astype(np.float32),
            (stored > 0.85 * storage_capacity_mb).astype(np.float32),
            (state_final[:, STATE_INDEX["battery_soc"]] < 0.25).astype(np.float32),
            (state_final[:, STATE_INDEX["ground_pass_active"]] > 0.5).astype(np.float32),
        ],
        axis=1,
    )
    return np.concatenate([delta_arr, reward[:, None].astype(np.float32), binary], axis=1)


def _build_features(
    model: Any,
    cfg: Any,
    dataset: dict[str, np.ndarray],
    dataset_path: Path,
    batch_size: int,
    max_samples: int | None,
    seed: int,
    device: torch.device,
) -> FeatureBundle:
    obs = dataset["obs"].astype(np.float32)
    action = dataset["action"].astype(np.float32)
    state = dataset["state"].astype(np.float32)
    reward = dataset["reward"].astype(np.float32)
    mode = dataset["mode"].astype(np.int64)
    scenario = dataset.get("scenario", np.asarray(["unknown"] * obs.shape[0])).astype(str)

    history = int(cfg.history_size)
    num_preds = int(cfg.num_preds)
    window = int(cfg.data.window)
    normalizers = fit_normalizers(dataset_path)
    obs_norm = normalize_obs(obs, normalizers)
    action_norm = normalize_action(action, normalizers)
    index = [(e, s) for e in range(obs.shape[0]) for s in range(obs.shape[1] - window + 1)]
    if max_samples is not None and max_samples > 0 and max_samples < len(index):
        rng = np.random.default_rng(seed)
        selected = np.sort(rng.choice(len(index), size=int(max_samples), replace=False))
        index = [index[int(i)] for i in selected]

    pred_parts, target_parts, y_parts, zero_parts = [], [], [], []
    ep_parts, step_parts, mode_parts, scenario_parts = [], [], [], []
    pred_len = 0

    with torch.no_grad():
        for start in range(0, len(index), batch_size):
            chunk = index[start : start + batch_size]
            obs_windows = np.stack([obs_norm[e, s : s + window] for e, s in chunk]).astype(np.float32)
            act_windows = np.stack([action_norm[e, s : s + window] for e, s in chunk]).astype(np.float32)
            batch = {
                "obs": torch.from_numpy(obs_windows).to(device),
                "action": torch.from_numpy(act_windows).to(device),
            }
            encoded = model.encode(batch)
            emb = encoded["emb"]
            act_emb = encoded["act_emb"]
            pred = model.predict(emb[:, :history], act_emb[:, :history])
            target = emb[:, num_preds:]
            m = min(pred.size(1), target.size(1))
            pred_len = int(m)
            pred_parts.append(pred[:, m - 1].cpu().numpy().astype(np.float32))
            target_parts.append(target[:, m - 1].cpu().numpy().astype(np.float32))

            episodes = np.asarray([e for e, _ in chunk], dtype=np.int64)
            final_offsets = np.asarray([s + num_preds + m - 1 for _, s in chunk], dtype=np.int64)
            prev_offsets = np.asarray([s + history - 1 for _, s in chunk], dtype=np.int64)
            y = _build_target(
                state_final=state[episodes, final_offsets],
                state_prev=state[episodes, prev_offsets],
                reward=reward[episodes, final_offsets],
            )
            zero = np.zeros_like(y)
            zero[:, len(CONTINUOUS_TARGETS) :] = y[:, len(CONTINUOUS_TARGETS) :].mean(axis=0, keepdims=True)
            y_parts.append(y)
            zero_parts.append(zero)
            ep_parts.append(episodes)
            step_parts.append(final_offsets)
            mode_parts.append(mode[episodes, final_offsets])
            scenario_parts.append(scenario[episodes])

    return FeatureBundle(
        pred_x=np.concatenate(pred_parts, axis=0),
        target_x=np.concatenate(target_parts, axis=0),
        target_y=np.concatenate(y_parts, axis=0),
        zero_y=np.concatenate(zero_parts, axis=0),
        episode=np.concatenate(ep_parts, axis=0),
        step=np.concatenate(step_parts, axis=0),
        mode=np.concatenate(mode_parts, axis=0),
        scenario=np.concatenate(scenario_parts, axis=0),
        history_size=history,
        num_preds=num_preds,
        pred_len=pred_len,
    )


def _predict(
    decoder: EventSatStateDecoder,
    x: np.ndarray,
    feature_mean: np.ndarray,
    feature_std: np.ndarray,
    target_mean: np.ndarray,
    target_std: np.ndarray,
    batch_size: int,
    device: torch.device,
) -> np.ndarray:
    outputs = []
    decoder.eval()
    with torch.no_grad():
        for start in range(0, x.shape[0], batch_size):
            xb = (x[start : start + batch_size] - feature_mean) / feature_std
            yb = decoder(torch.from_numpy(xb).to(device)).cpu().numpy()
            outputs.append(yb * target_std + target_mean)
    return np.concatenate(outputs, axis=0)


def _regression_metrics(pred: np.ndarray, target: np.ndarray, names: tuple[str, ...]) -> dict[str, Any]:
    residual = pred - target
    rmse = np.sqrt(np.mean(residual**2, axis=0))
    mae = np.mean(np.abs(residual), axis=0)
    return {
        "rmse": {name: float(rmse[i]) for i, name in enumerate(names)},
        "mae": {name: float(mae[i]) for i, name in enumerate(names)},
    }


def _binary_metrics(pred: np.ndarray, target: np.ndarray) -> dict[str, dict[str, float]]:
    out = {}
    for i, name in enumerate(BINARY_TARGETS):
        p = pred[:, i] >= 0.5
        y = target[:, i] >= 0.5
        tp = int(np.sum(p & y))
        fp = int(np.sum(p & ~y))
        fn = int(np.sum(~p & y))
        tn = int(np.sum(~p & ~y))
        precision = tp / max(1, tp + fp)
        recall = tp / max(1, tp + fn)
        f1 = 2.0 * precision * recall / max(1e-12, precision + recall)
        out[name] = {
            "accuracy": float((tp + tn) / max(1, tp + fp + fn + tn)),
            "precision": float(precision),
            "recall": float(recall),
            "f1": float(f1),
        }
    return out


def _metrics(pred: np.ndarray, target: np.ndarray) -> dict[str, Any]:
    n_cont = len(CONTINUOUS_TARGETS)
    return {
        "continuous": _regression_metrics(pred[:, :n_cont], target[:, :n_cont], CONTINUOUS_TARGETS),
        "binary": _binary_metrics(pred[:, n_cont:], target[:, n_cont:]),
    }


def _per_mode(pred: np.ndarray, target: np.ndarray, mode: np.ndarray) -> list[dict[str, Any]]:
    rows = []
    n_cont = len(CONTINUOUS_TARGETS)
    for idx, name in enumerate(LITE_MODE_LIST):
        mask = mode == idx
        if not np.any(mask):
            continue
        metrics = _regression_metrics(pred[mask, :n_cont], target[mask, :n_cont], CONTINUOUS_TARGETS)
        rows.append(
            {
                "mode": name,
                "samples": int(mask.sum()),
                "delta_downlinked_rmse": metrics["rmse"]["delta_data_downlinked_mb"],
                "reward_rmse": metrics["rmse"]["reward"],
            }
        )
    return rows


def train(args: argparse.Namespace) -> dict[str, Any]:
    torch.backends.nnpack.enabled = False
    torch.set_num_threads(max(1, min(torch.get_num_threads(), args.torch_threads)))
    torch.manual_seed(args.seed)
    np.random.seed(args.seed)
    device = torch.device(args.device)

    dataset_path = Path(args.dataset)
    if not dataset_path.is_absolute():
        dataset_path = ROOT / dataset_path
    dataset = {key: value for key, value in np.load(dataset_path).items()}

    load_start = _timer()
    model, cfg, run = load_eventsat_model(device=device, dataset=MODEL_DATASET)
    load_time_s = _elapsed(load_start)
    feature_start = _timer()
    features = _build_features(model, cfg, dataset, dataset_path, args.feature_batch_size, args.max_samples, args.seed, device)
    feature_time_s = _elapsed(feature_start)

    episodes = np.unique(features.episode)
    rng = np.random.default_rng(args.seed)
    rng.shuffle(episodes)
    n_eval_eps = max(1, int(math.ceil(len(episodes) * args.eval_fraction)))
    eval_episodes = np.sort(episodes[:n_eval_eps])
    eval_mask = np.isin(features.episode, eval_episodes)
    train_mask = ~eval_mask

    train_x_raw = np.concatenate([features.target_x[train_mask], features.pred_x[train_mask]], axis=0)
    train_y_raw = np.concatenate([features.target_y[train_mask], features.target_y[train_mask]], axis=0)
    feature_mean, feature_std = _fit_standardizer(train_x_raw)
    target_mean, target_std = _fit_standardizer(train_y_raw)
    train_x = (train_x_raw - feature_mean) / feature_std
    train_y = (train_y_raw - target_mean) / target_std

    perm = rng.permutation(train_x.shape[0])
    n_val = max(1, int(args.validation_fraction * len(perm)))
    val_idx = perm[:n_val]
    fit_idx = perm[n_val:]
    train_loader = DataLoader(
        TensorDataset(torch.from_numpy(train_x[fit_idx]), torch.from_numpy(train_y[fit_idx])),
        batch_size=args.batch_size,
        shuffle=True,
    )
    val_loader = DataLoader(
        TensorDataset(torch.from_numpy(train_x[val_idx]), torch.from_numpy(train_y[val_idx])),
        batch_size=args.batch_size,
        shuffle=False,
    )

    decoder = EventSatStateDecoder(train_x.shape[1], args.hidden_dim, args.depth, train_y.shape[1]).to(device)
    optimizer = torch.optim.AdamW(decoder.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    loss_fn = nn.MSELoss()
    best_val = math.inf
    best_state = None
    history = []
    train_start = _timer()
    for epoch in range(args.epochs):
        decoder.train()
        train_loss = 0.0
        train_n = 0
        for xb, yb in train_loader:
            xb = xb.to(device)
            yb = yb.to(device)
            optimizer.zero_grad(set_to_none=True)
            loss = loss_fn(decoder(xb), yb)
            loss.backward()
            optimizer.step()
            train_loss += float(loss.item()) * xb.size(0)
            train_n += xb.size(0)
        train_loss /= max(1, train_n)

        decoder.eval()
        val_loss = 0.0
        val_n = 0
        with torch.no_grad():
            for xb, yb in val_loader:
                loss = loss_fn(decoder(xb.to(device)), yb.to(device))
                val_loss += float(loss.item()) * xb.size(0)
                val_n += xb.size(0)
        val_loss /= max(1, val_n)
        history.append({"epoch": float(epoch), "train_loss": train_loss, "val_loss": val_loss})
        if val_loss < best_val:
            best_val = val_loss
            best_state = {key: value.detach().cpu().clone() for key, value in decoder.state_dict().items()}
        if epoch == 0 or (epoch + 1) % args.log_every == 0 or epoch == args.epochs - 1:
            print(f"epoch {epoch + 1:03d}/{args.epochs} train_loss={train_loss:.6f} val_loss={val_loss:.6f}", flush=True)

    if best_state is not None:
        decoder.load_state_dict(best_state)
    train_time_s = _elapsed(train_start)

    eval_y = features.target_y[eval_mask]
    pred_decoded = _predict(decoder, features.pred_x[eval_mask], feature_mean, feature_std, target_mean, target_std, args.batch_size, device)
    target_decoded = _predict(decoder, features.target_x[eval_mask], feature_mean, feature_std, target_mean, target_std, args.batch_size, device)
    zero_baseline = features.zero_y[eval_mask]

    created_at = datetime.now(timezone.utc).isoformat()
    out_path = Path(args.out)
    metrics_path = Path(args.metrics_out)
    artifact = {
        "state_dict": {key: value.detach().cpu() for key, value in decoder.state_dict().items()},
        "input_dim": int(train_x.shape[1]),
        "hidden_dim": int(args.hidden_dim),
        "depth": int(args.depth),
        "output_dim": int(train_y.shape[1]),
        "feature_mean": torch.tensor(feature_mean),
        "feature_std": torch.tensor(feature_std),
        "target_mean": torch.tensor(target_mean),
        "target_std": torch.tensor(target_std),
        "target_names": list(TARGET_NAMES),
        "continuous_target_names": list(CONTINUOUS_TARGETS),
        "binary_target_names": list(BINARY_TARGETS),
        "history_size": int(features.history_size),
        "num_preds": int(features.num_preds),
        "pred_len": int(features.pred_len),
        "dataset": relpath(dataset_path),
        "run": run,
        "created_at": created_at,
    }
    out_path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(artifact, out_path)

    pred_metrics = _metrics(pred_decoded, eval_y)
    target_metrics = _metrics(target_decoded, eval_y)
    zero_metrics = _metrics(zero_baseline, eval_y)
    downlink_pred_rmse = pred_metrics["continuous"]["rmse"]["delta_data_downlinked_mb"]
    downlink_zero_rmse = zero_metrics["continuous"]["rmse"]["delta_data_downlinked_mb"]
    metrics = {
        "ok": True,
        "generated_at": created_at,
        "artifact": relpath(out_path),
        "dataset": relpath(dataset_path),
        "run": run,
        "samples": {
            "total_windows": int(features.target_y.shape[0]),
            "train_windows": int(np.sum(train_mask)),
            "eval_windows": int(np.sum(eval_mask)),
            "eval_episodes": eval_episodes.astype(int).tolist(),
        },
        "target_names": list(TARGET_NAMES),
        "continuous_target_names": list(CONTINUOUS_TARGETS),
        "binary_target_names": list(BINARY_TARGETS),
        "best_val_loss": float(best_val),
        "load_time_s": float(load_time_s),
        "feature_time_s": float(feature_time_s),
        "train_time_s": float(train_time_s),
        "history": history,
        "predicted_latent_decode": pred_metrics,
        "target_latent_decode": target_metrics,
        "zero_delta_baseline": zero_metrics,
        "acceptance": {
            "predicted_downlink_delta_beats_zero": bool(downlink_pred_rmse < downlink_zero_rmse),
            "target_continuous_beats_zero_on_all": bool(
                all(
                    target_metrics["continuous"]["rmse"][name] < zero_metrics["continuous"]["rmse"][name]
                    for name in CONTINUOUS_TARGETS
                )
            ),
        },
        "per_mode_error": {
            "predicted_latent_decode": _per_mode(pred_decoded, eval_y, features.mode[eval_mask]),
            "target_latent_decode": _per_mode(target_decoded, eval_y, features.mode[eval_mask]),
        },
    }
    write_json(metrics_path, metrics)
    print(
        f"wrote {relpath(out_path)} and {relpath(metrics_path)} "
        f"delta_downlink_rmse={downlink_pred_rmse:.4g}",
        flush=True,
    )
    return metrics


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset", default=str(DATASET))
    parser.add_argument("--out", default=str(OUT))
    parser.add_argument("--metrics-out", default=str(METRICS_OUT))
    parser.add_argument("--epochs", type=int, default=40)
    parser.add_argument("--batch-size", type=int, default=2048)
    parser.add_argument("--feature-batch-size", type=int, default=512)
    parser.add_argument("--hidden-dim", type=int, default=256)
    parser.add_argument("--depth", type=int, default=2)
    parser.add_argument("--lr", type=float, default=1e-3)
    parser.add_argument("--weight-decay", type=float, default=1e-4)
    parser.add_argument("--eval-fraction", type=float, default=0.2)
    parser.add_argument("--validation-fraction", type=float, default=0.1)
    parser.add_argument("--max-samples", type=int, default=0)
    parser.add_argument("--seed", type=int, default=3072)
    parser.add_argument("--device", default="cpu")
    parser.add_argument("--torch-threads", type=int, default=8)
    parser.add_argument("--log-every", type=int, default=10)
    args = parser.parse_args()
    args.max_samples = args.max_samples if args.max_samples and args.max_samples > 0 else None
    train(args)


if __name__ == "__main__":
    main()
