...
def _write_dataset(path):
    ...
    action = np.zeros((E, T, 7), dtype=np.float32)
    action[..., 0] = 1.0
...
class FakeLatentModel:
    def rollout(self, history, action):
        mode_signal = action[..., :7] @ np.arange(7, dtype=np.float32)
        z0 = mode_signal / 6.0
        z1 = np.cumsum(action[..., 0], axis=1)
        z2 = np.cumsum(action[..., 2], axis=1)
        return np.stack([z0, z1, z2], axis=-1).astype(np.float32)


def test_cem_respects_first_mask():
    from swm_eventsat.planning import CEMPlanner, default_mode_weights
    ...
    mask[1] = False
    history = {"obs": np.zeros((4, 25), dtype=np.float32), "action": np.zeros((4, 7), dtype=np.float32)}
