...
from swm_eventsat.data.toy_eventsat_env import MODE_LIST, STATE_NAMES
...
def _is_eventsat_run(run: dict[str, Any]) -> bool:
    ...
    return (
        ...
        and (model_target is None or model_target in ("core.models.vector_jepa.VectorJEPA", "models.od_jepa.ODJEPA"))
    )
...
<head>
  ...
  <style>
    /* autops-style theme overlay (Computer Modern + TUM blue), matches ~/autops-agentic-framework */
    body {{ font-family:'Computer Modern Serif',Georgia,'Times New Roman',serif !important; background:#fff !important; }}
    header {{ border-bottom:2px solid #0065BD !important; background:#fff !important; }}
    h1 {{ color:#0065BD !important; font-weight:600 !important; }}
    h2 {{ color:#005293 !important; font-weight:700 !important; }}
    .sub, .label {{ font-family:'Computer Modern Sans',Arial,sans-serif !important; }}
    .value {{ color:#0065BD !important; }}
    table {{ font-family:'Computer Modern Serif',Georgia,serif !important; }}
    th {{ background:#fff !important; color:#111 !important; border-top:2px solid #111 !important; border-bottom:1px solid #111 !important; font-family:'Computer Modern Sans',Arial,sans-serif !important; }}
  </style>
  <style>
...
const layoutBase = {{margin:{{t:18,b:48,l:58,r:58}}, paper_bgcolor:"#fff", plot_bgcolor:"#fff", legend:{{orientation:"h", y:-0.22}}, font:{{family:"'Computer Modern Sans', Arial, sans-serif", size:12}}}};
